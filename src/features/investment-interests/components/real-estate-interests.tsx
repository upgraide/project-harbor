...
import { useGetAllRealEstateInterests } from "../hooks/use-interests";
import { backofficeRealEstatePath } from "@/paths";

type RealEstateInterestsProps = {
...
export const RealEstateInterests = ({ opportunityId }: RealEstateInterestsProps) => {
  const { data: interests } = useGetAllRealEstateInterests(opportunityId);
  ...
  return (
    <main className="flex max-w-screen-xs flex-1 flex-col space-y-6 px-6 py-4 md:mx-auto md:max-w-screen-xl md:px-4">
      <div className="flex items-center justify-between">
        ...
        <Link href={`${backofficeRealEstatePath()}/${opportunityId}`}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-bold text-2xl">{notInterestedUsers.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-3">
              ...
              <TableBody>
                {interestedUsers.map((interest) => (
                  <TableRow key={interest.id}>
                    <TableCell className="font-medium">{interest.user.name}</TableCell>
      ...
      {interests.length === 0 && (
        <Card>
          <CardContent className="flex min-h-[200px] items-center justify-center">
            <p className="text-muted-foreground">No user interests recorded yet</p>
          </CardContent>
        </Card>
      )}
    </main>
